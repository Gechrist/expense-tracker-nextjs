...
export const to24HrTime = (time: string) => {
  let [hr, min] =
    String(time)
      .toLowerCase()
      .match(/\d+|[a-z]+/g) || [];
  ...
  return `${ap == 'AM' && hr?.length === 1 ? '0' : ''}${
    ((hr as unknown as number) % 12) + (ap == 'AM' ? 0 : 12)
  }:${min}:00`;
};

export const modifyDateString = (type: string, date: string): string => {
  // if type is in English, the locale will be also in English
